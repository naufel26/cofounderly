import { queryParams, type RouteQueryOptions, type RouteDefinition, type RouteFormDefinition } from './../wayfinder'
/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::login
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:47
* @route '/login'
*/
export const login = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: login.url(options),
    method: 'get',
})

login.definition = {
    methods: ["get","head"],
    url: '/login',
} satisfies RouteDefinition<["get","head"]>

/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::login
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:47
* @route '/login'
*/
login.url = (options?: RouteQueryOptions) => {
    return login.definition.url + queryParams(options)
}

/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::login
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:47
* @route '/login'
*/
login.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: login.url(options),
    method: 'get',
})

/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::login
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:47
* @route '/login'
*/
login.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: login.url(options),
    method: 'head',
})

/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::login
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:47
* @route '/login'
*/
const loginForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: login.url(options),
    method: 'get',
})

/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::login
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:47
* @route '/login'
*/
loginForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: login.url(options),
    method: 'get',
})

/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::login
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:47
* @route '/login'
*/
loginForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: login.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

login.form = loginForm

/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::logout
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:100
* @route '/logout'
*/
export const logout = (options?: RouteQueryOptions): RouteDefinition<'post'> => ({
    url: logout.url(options),
    method: 'post',
})

logout.definition = {
    methods: ["post"],
    url: '/logout',
} satisfies RouteDefinition<["post"]>

/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::logout
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:100
* @route '/logout'
*/
logout.url = (options?: RouteQueryOptions) => {
    return logout.definition.url + queryParams(options)
}

/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::logout
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:100
* @route '/logout'
*/
logout.post = (options?: RouteQueryOptions): RouteDefinition<'post'> => ({
    url: logout.url(options),
    method: 'post',
})

/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::logout
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:100
* @route '/logout'
*/
const logoutForm = (options?: RouteQueryOptions): RouteFormDefinition<'post'> => ({
    action: logout.url(options),
    method: 'post',
})

/**
* @see \Laravel\Fortify\Http\Controllers\AuthenticatedSessionController::logout
* @see vendor/laravel/fortify/src/Http/Controllers/AuthenticatedSessionController.php:100
* @route '/logout'
*/
logoutForm.post = (options?: RouteQueryOptions): RouteFormDefinition<'post'> => ({
    action: logout.url(options),
    method: 'post',
})

logout.form = logoutForm

/**
* @see \Laravel\Fortify\Http\Controllers\RegisteredUserController::register
* @see vendor/laravel/fortify/src/Http/Controllers/RegisteredUserController.php:41
* @route '/register'
*/
export const register = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: register.url(options),
    method: 'get',
})

register.definition = {
    methods: ["get","head"],
    url: '/register',
} satisfies RouteDefinition<["get","head"]>

/**
* @see \Laravel\Fortify\Http\Controllers\RegisteredUserController::register
* @see vendor/laravel/fortify/src/Http/Controllers/RegisteredUserController.php:41
* @route '/register'
*/
register.url = (options?: RouteQueryOptions) => {
    return register.definition.url + queryParams(options)
}

/**
* @see \Laravel\Fortify\Http\Controllers\RegisteredUserController::register
* @see vendor/laravel/fortify/src/Http/Controllers/RegisteredUserController.php:41
* @route '/register'
*/
register.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: register.url(options),
    method: 'get',
})

/**
* @see \Laravel\Fortify\Http\Controllers\RegisteredUserController::register
* @see vendor/laravel/fortify/src/Http/Controllers/RegisteredUserController.php:41
* @route '/register'
*/
register.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: register.url(options),
    method: 'head',
})

/**
* @see \Laravel\Fortify\Http\Controllers\RegisteredUserController::register
* @see vendor/laravel/fortify/src/Http/Controllers/RegisteredUserController.php:41
* @route '/register'
*/
const registerForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: register.url(options),
    method: 'get',
})

/**
* @see \Laravel\Fortify\Http\Controllers\RegisteredUserController::register
* @see vendor/laravel/fortify/src/Http/Controllers/RegisteredUserController.php:41
* @route '/register'
*/
registerForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: register.url(options),
    method: 'get',
})

/**
* @see \Laravel\Fortify\Http\Controllers\RegisteredUserController::register
* @see vendor/laravel/fortify/src/Http/Controllers/RegisteredUserController.php:41
* @route '/register'
*/
registerForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: register.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

register.form = registerForm

/**
* @see routes/web.php:21
* @route '/laravel'
*/
export const home = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: home.url(options),
    method: 'get',
})

home.definition = {
    methods: ["get","head"],
    url: '/laravel',
} satisfies RouteDefinition<["get","head"]>

/**
* @see routes/web.php:21
* @route '/laravel'
*/
home.url = (options?: RouteQueryOptions) => {
    return home.definition.url + queryParams(options)
}

/**
* @see routes/web.php:21
* @route '/laravel'
*/
home.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: home.url(options),
    method: 'get',
})

/**
* @see routes/web.php:21
* @route '/laravel'
*/
home.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: home.url(options),
    method: 'head',
})

/**
* @see routes/web.php:21
* @route '/laravel'
*/
const homeForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: home.url(options),
    method: 'get',
})

/**
* @see routes/web.php:21
* @route '/laravel'
*/
homeForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: home.url(options),
    method: 'get',
})

/**
* @see routes/web.php:21
* @route '/laravel'
*/
homeForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: home.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

home.form = homeForm

/**
* @see routes/web.php:27
* @route '/'
*/
export const index = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: index.url(options),
    method: 'get',
})

index.definition = {
    methods: ["get","head"],
    url: '/',
} satisfies RouteDefinition<["get","head"]>

/**
* @see routes/web.php:27
* @route '/'
*/
index.url = (options?: RouteQueryOptions) => {
    return index.definition.url + queryParams(options)
}

/**
* @see routes/web.php:27
* @route '/'
*/
index.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: index.url(options),
    method: 'get',
})

/**
* @see routes/web.php:27
* @route '/'
*/
index.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: index.url(options),
    method: 'head',
})

/**
* @see routes/web.php:27
* @route '/'
*/
const indexForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: index.url(options),
    method: 'get',
})

/**
* @see routes/web.php:27
* @route '/'
*/
indexForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: index.url(options),
    method: 'get',
})

/**
* @see routes/web.php:27
* @route '/'
*/
indexForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: index.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

index.form = indexForm

/**
* @see routes/web.php:31
* @route '/signup'
*/
export const signup = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: signup.url(options),
    method: 'get',
})

signup.definition = {
    methods: ["get","head"],
    url: '/signup',
} satisfies RouteDefinition<["get","head"]>

/**
* @see routes/web.php:31
* @route '/signup'
*/
signup.url = (options?: RouteQueryOptions) => {
    return signup.definition.url + queryParams(options)
}

/**
* @see routes/web.php:31
* @route '/signup'
*/
signup.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: signup.url(options),
    method: 'get',
})

/**
* @see routes/web.php:31
* @route '/signup'
*/
signup.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: signup.url(options),
    method: 'head',
})

/**
* @see routes/web.php:31
* @route '/signup'
*/
const signupForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: signup.url(options),
    method: 'get',
})

/**
* @see routes/web.php:31
* @route '/signup'
*/
signupForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: signup.url(options),
    method: 'get',
})

/**
* @see routes/web.php:31
* @route '/signup'
*/
signupForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: signup.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

signup.form = signupForm

/**
* @see \App\Http\Controllers\OnboardingController::onboarding
* @see app/Http/Controllers/OnboardingController.php:12
* @route '/onboarding'
*/
export const onboarding = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: onboarding.url(options),
    method: 'get',
})

onboarding.definition = {
    methods: ["get","head"],
    url: '/onboarding',
} satisfies RouteDefinition<["get","head"]>

/**
* @see \App\Http\Controllers\OnboardingController::onboarding
* @see app/Http/Controllers/OnboardingController.php:12
* @route '/onboarding'
*/
onboarding.url = (options?: RouteQueryOptions) => {
    return onboarding.definition.url + queryParams(options)
}

/**
* @see \App\Http\Controllers\OnboardingController::onboarding
* @see app/Http/Controllers/OnboardingController.php:12
* @route '/onboarding'
*/
onboarding.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: onboarding.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OnboardingController::onboarding
* @see app/Http/Controllers/OnboardingController.php:12
* @route '/onboarding'
*/
onboarding.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: onboarding.url(options),
    method: 'head',
})

/**
* @see \App\Http\Controllers\OnboardingController::onboarding
* @see app/Http/Controllers/OnboardingController.php:12
* @route '/onboarding'
*/
const onboardingForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: onboarding.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OnboardingController::onboarding
* @see app/Http/Controllers/OnboardingController.php:12
* @route '/onboarding'
*/
onboardingForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: onboarding.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OnboardingController::onboarding
* @see app/Http/Controllers/OnboardingController.php:12
* @route '/onboarding'
*/
onboardingForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: onboarding.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

onboarding.form = onboardingForm

/**
* @see routes/web.php:55
* @route '/dashboard'
*/
export const dashboard = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: dashboard.url(options),
    method: 'get',
})

dashboard.definition = {
    methods: ["get","head"],
    url: '/dashboard',
} satisfies RouteDefinition<["get","head"]>

/**
* @see routes/web.php:55
* @route '/dashboard'
*/
dashboard.url = (options?: RouteQueryOptions) => {
    return dashboard.definition.url + queryParams(options)
}

/**
* @see routes/web.php:55
* @route '/dashboard'
*/
dashboard.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: dashboard.url(options),
    method: 'get',
})

/**
* @see routes/web.php:55
* @route '/dashboard'
*/
dashboard.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: dashboard.url(options),
    method: 'head',
})

/**
* @see routes/web.php:55
* @route '/dashboard'
*/
const dashboardForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: dashboard.url(options),
    method: 'get',
})

/**
* @see routes/web.php:55
* @route '/dashboard'
*/
dashboardForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: dashboard.url(options),
    method: 'get',
})

/**
* @see routes/web.php:55
* @route '/dashboard'
*/
dashboardForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: dashboard.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

dashboard.form = dashboardForm

/**
* @see \App\Http\Controllers\FeedController::feeds
* @see app/Http/Controllers/FeedController.php:12
* @route '/feeds'
*/
export const feeds = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: feeds.url(options),
    method: 'get',
})

feeds.definition = {
    methods: ["get","head"],
    url: '/feeds',
} satisfies RouteDefinition<["get","head"]>

/**
* @see \App\Http\Controllers\FeedController::feeds
* @see app/Http/Controllers/FeedController.php:12
* @route '/feeds'
*/
feeds.url = (options?: RouteQueryOptions) => {
    return feeds.definition.url + queryParams(options)
}

/**
* @see \App\Http\Controllers\FeedController::feeds
* @see app/Http/Controllers/FeedController.php:12
* @route '/feeds'
*/
feeds.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: feeds.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\FeedController::feeds
* @see app/Http/Controllers/FeedController.php:12
* @route '/feeds'
*/
feeds.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: feeds.url(options),
    method: 'head',
})

/**
* @see \App\Http\Controllers\FeedController::feeds
* @see app/Http/Controllers/FeedController.php:12
* @route '/feeds'
*/
const feedsForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: feeds.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\FeedController::feeds
* @see app/Http/Controllers/FeedController.php:12
* @route '/feeds'
*/
feedsForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: feeds.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\FeedController::feeds
* @see app/Http/Controllers/FeedController.php:12
* @route '/feeds'
*/
feedsForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: feeds.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

feeds.form = feedsForm

/**
* @see \App\Http\Controllers\SearchController::__invoke
* @see app/Http/Controllers/SearchController.php:14
* @route '/search'
*/
export const search = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: search.url(options),
    method: 'get',
})

search.definition = {
    methods: ["get","head"],
    url: '/search',
} satisfies RouteDefinition<["get","head"]>

/**
* @see \App\Http\Controllers\SearchController::__invoke
* @see app/Http/Controllers/SearchController.php:14
* @route '/search'
*/
search.url = (options?: RouteQueryOptions) => {
    return search.definition.url + queryParams(options)
}

/**
* @see \App\Http\Controllers\SearchController::__invoke
* @see app/Http/Controllers/SearchController.php:14
* @route '/search'
*/
search.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: search.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\SearchController::__invoke
* @see app/Http/Controllers/SearchController.php:14
* @route '/search'
*/
search.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: search.url(options),
    method: 'head',
})

/**
* @see \App\Http\Controllers\SearchController::__invoke
* @see app/Http/Controllers/SearchController.php:14
* @route '/search'
*/
const searchForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: search.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\SearchController::__invoke
* @see app/Http/Controllers/SearchController.php:14
* @route '/search'
*/
searchForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: search.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\SearchController::__invoke
* @see app/Http/Controllers/SearchController.php:14
* @route '/search'
*/
searchForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: search.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

search.form = searchForm

/**
* @see \App\Http\Controllers\AdvisorController::__invoke
* @see app/Http/Controllers/AdvisorController.php:13
* @route '/advisors'
*/
export const advisors = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: advisors.url(options),
    method: 'get',
})

advisors.definition = {
    methods: ["get","head"],
    url: '/advisors',
} satisfies RouteDefinition<["get","head"]>

/**
* @see \App\Http\Controllers\AdvisorController::__invoke
* @see app/Http/Controllers/AdvisorController.php:13
* @route '/advisors'
*/
advisors.url = (options?: RouteQueryOptions) => {
    return advisors.definition.url + queryParams(options)
}

/**
* @see \App\Http\Controllers\AdvisorController::__invoke
* @see app/Http/Controllers/AdvisorController.php:13
* @route '/advisors'
*/
advisors.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: advisors.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\AdvisorController::__invoke
* @see app/Http/Controllers/AdvisorController.php:13
* @route '/advisors'
*/
advisors.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: advisors.url(options),
    method: 'head',
})

/**
* @see \App\Http\Controllers\AdvisorController::__invoke
* @see app/Http/Controllers/AdvisorController.php:13
* @route '/advisors'
*/
const advisorsForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: advisors.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\AdvisorController::__invoke
* @see app/Http/Controllers/AdvisorController.php:13
* @route '/advisors'
*/
advisorsForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: advisors.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\AdvisorController::__invoke
* @see app/Http/Controllers/AdvisorController.php:13
* @route '/advisors'
*/
advisorsForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: advisors.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

advisors.form = advisorsForm

/**
* @see routes/web.php:85
* @route '/meetings'
*/
export const meetings = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: meetings.url(options),
    method: 'get',
})

meetings.definition = {
    methods: ["get","head"],
    url: '/meetings',
} satisfies RouteDefinition<["get","head"]>

/**
* @see routes/web.php:85
* @route '/meetings'
*/
meetings.url = (options?: RouteQueryOptions) => {
    return meetings.definition.url + queryParams(options)
}

/**
* @see routes/web.php:85
* @route '/meetings'
*/
meetings.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: meetings.url(options),
    method: 'get',
})

/**
* @see routes/web.php:85
* @route '/meetings'
*/
meetings.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: meetings.url(options),
    method: 'head',
})

/**
* @see routes/web.php:85
* @route '/meetings'
*/
const meetingsForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: meetings.url(options),
    method: 'get',
})

/**
* @see routes/web.php:85
* @route '/meetings'
*/
meetingsForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: meetings.url(options),
    method: 'get',
})

/**
* @see routes/web.php:85
* @route '/meetings'
*/
meetingsForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: meetings.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

meetings.form = meetingsForm

/**
* @see \App\Http\Controllers\MessagesController::__invoke
* @see app/Http/Controllers/MessagesController.php:11
* @route '/messages'
*/
export const messages = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: messages.url(options),
    method: 'get',
})

messages.definition = {
    methods: ["get","head"],
    url: '/messages',
} satisfies RouteDefinition<["get","head"]>

/**
* @see \App\Http\Controllers\MessagesController::__invoke
* @see app/Http/Controllers/MessagesController.php:11
* @route '/messages'
*/
messages.url = (options?: RouteQueryOptions) => {
    return messages.definition.url + queryParams(options)
}

/**
* @see \App\Http\Controllers\MessagesController::__invoke
* @see app/Http/Controllers/MessagesController.php:11
* @route '/messages'
*/
messages.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: messages.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\MessagesController::__invoke
* @see app/Http/Controllers/MessagesController.php:11
* @route '/messages'
*/
messages.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: messages.url(options),
    method: 'head',
})

/**
* @see \App\Http\Controllers\MessagesController::__invoke
* @see app/Http/Controllers/MessagesController.php:11
* @route '/messages'
*/
const messagesForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: messages.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\MessagesController::__invoke
* @see app/Http/Controllers/MessagesController.php:11
* @route '/messages'
*/
messagesForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: messages.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\MessagesController::__invoke
* @see app/Http/Controllers/MessagesController.php:11
* @route '/messages'
*/
messagesForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: messages.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

messages.form = messagesForm
