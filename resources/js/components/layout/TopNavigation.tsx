...
import { ChevronDown, Home, MessageSquare, Sparkles } from 'lucide-react';
...
export const TopNavigation = () => {
    ...
    const navIconBase = 'relative flex h-10 w-10 items-center justify-center rounded-xl transition-colors';
    const navIconActive = 'bg-[#E6F6F4] text-[#2DAB94]';
    const navIconInactive = 'text-slate-500 hover:bg-slate-100 hover:text-slate-700';

    return (
        <header className="border-border/60 fixed left-0 right-0 top-0 z-50 border-b bg-white/80 backdrop-blur-md">
            <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4">
                {/* Brand Logo */}
                <Link href="/feeds" className="flex shrink-0 items-center gap-2.5">
                ...
                <nav className="flex items-center gap-1.5">
                    ...
                    <Link
                        href="/feeds"
                        className={`${navIconBase} ${isOnFeeds ? navIconActive : navIconInactive}`}
                        title="Home"
                    >
                    ...
                    >
                        ...
                        {chatUnread > 0 && (
                            <span className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full border-2 border-white bg-[#2DAB94] text-[10px] font-bold text-white">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <button className="ml-2 flex items-center gap-1 rounded-full p-0.5 transition-colors hover:bg-slate-100">
